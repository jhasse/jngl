...
namespace jngl {
	...
	Window::Window(const std::string& title, const int width, const int height, const bool fullscreen)
		: fullscreen_(fullscreen), running_(false), isMouseVisible_(true), relativeMouseMode(false),
		  ...
		  oldTime(0), changeWork_(false), stepsPerFrame(1), multitouch(false)
	{
	...
	void Window::SetIcon(const std::string& filename) {
		...
		png_read_png(png_ptr, info_ptr, PNG_TRANSFORM_EXPAND | PNG_TRANSFORM_STRIP_16 | PNG_TRANSFORM_BGR, NULL);
