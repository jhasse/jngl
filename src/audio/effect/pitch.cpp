...
struct pitch_control_impl : pitch_control {
	...
	: stream_(std::move(stream)), resampler_(1.f / ratio, smoothness) {
	}
	...
	std::size_t read(float* data, std::size_t sample_count) override {
		...
		while (result < sample_count) {
			if (resampler_pos_ < resampler_.result().size()) {
				std::size_t size =
				    std::min(sample_count - result, resampler_.result().size() - resampler_pos_);
				...
				std::size_t request_size = std::max<std::size_t>(
				    sample_count, std::ceil(resampler_.ratio() * sample_count / 2.f) * 2);
