...
boost::ut::suite _ = [] { // NOLINT
	...
	"FrameLimiter_Vsync"_test = [] {
		...
		for (int i = 0; i < 9000; ++i) {
			...
			totalSteps += numberOfSteps;
			timing.time_us +=
			    2 * 1'000 + numberOfSteps * 3 * 1'000; // draw takes 2 ms, step takes 3 ms
			timing.swapBuffers();
