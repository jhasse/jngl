...
float XinputController::stateImpl(const controller b) const {
	const float max = 32767;

	switch (b) {
	case controller::LeftStickX:
		return float(states[i].Gamepad.sThumbLX) / max;
	case controller::LeftStickY:
		return float(states[i].Gamepad.sThumbLY) / max;
	case controller::RightStickX:
		return float(states[i].Gamepad.sThumbRX) / max;
	case controller::RightStickY:
		return float(states[i].Gamepad.sThumbRY) / max;
	case controller::A:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_A ? 1.0f : 0.0f;
	case controller::B:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_B ? 1.0f : 0.0f;
	case controller::X:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_X ? 1.0f : 0.0f;
	case controller::Y:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_Y ? 1.0f : 0.0f;
	case controller::LeftButton:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER ? 1.0f : 0.0f;
	case controller::RightButton:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER ? 1.0f : 0.0f;
	case controller::LeftTrigger:
		return float(states[i].Gamepad.bLeftTrigger) / 255;
	case controller::RightTrigger:
		return float(states[i].Gamepad.bRightTrigger) / 255;
	case controller::Start:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_START ? 1.0f : 0.0f;
	case controller::Back:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_BACK ? 1.0f : 0.0f;
	case controller::DpadUp:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_UP ? 1.0f : 0.0f;
	case controller::DpadDown:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_DOWN ? 1.0f : 0.0f;
	case controller::DpadLeft:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT ? 1.0f : 0.0f;
	case controller::DpadRight:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT ? 1.0f : 0.0f;
	case controller::LeftStick:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_THUMB ? 1.0f : 0.0f;
	case controller::RightStick:
		return states[i].Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_THUMB ? 1.0f : 0.0f;
	default:
		return 0.0f;
	}
}

bool XinputController::down(const controller b) const {
	return state(b) > 0.5f;
}
